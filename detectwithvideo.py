...
cap = cv2.VideoCapture("road_video2.mp4") #use this line to detect objects in a video file, replace "road_video.mp4" with the path to your video file
...
while cap.isOpened():#while camera is open
    ...
    #resizing the frame to a maximum width and height while maintaining aspect ratio to improve processing speed and reduce memory usage, you can adjust the max_width and max_height values as needed
    max_width = 800
    max_height = 600
    ...
    cv2.rectangle(frame, (0, 0), (frame_width, 80), (0, 0, 0), -1)  # taller bar
    cv2.putText(frame, f"DECISION: {decision}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, decision_color, 2)
    cv2.putText(frame, f"RISK: {total_risk}", (10, 65), cv2.FONT_HERSHEY_SIMPLEX, 0.9, decision_color, 2)
